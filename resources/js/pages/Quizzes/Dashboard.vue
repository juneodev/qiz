...
<template>
  <div class="mx-auto max-w-7xl px-6 pb-16 pt-4">
    <h1 class="mb-2 text-2xl font-semibold">Tableau de bord</h1>
    <p class="mb-8 text-white/70">Bienvenue, {{ props.user.name }} 👋</p>

    <!-- Stats -->
    <div class="mb-8 grid gap-4 sm:grid-cols-3">
      <div class="rounded-xl bg-white/5 p-5 ring-1 ring-white/10">
        <p class="text-sm text-white/70">Total des quizzes</p>
        <p class="mt-2 text-3xl font-bold">{{ props.stats.total }}</p>
      </div>
      <div class="rounded-xl bg-white/5 p-5 ring-1 ring-white/10">
        <p class="text-sm text-white/70">Publiés</p>
        <p class="mt-2 text-3xl font-bold text-emerald-300">{{ props.stats.published }}</p>
      </div>
      <div class="rounded-xl bg-white/5 p-5 ring-1 ring-white/10">
        <p class="text-sm text-white/70">Brouillons</p>
        <p class="mt-2 text-3xl font-bold text-amber-300">{{ props.stats.drafts }}</p>
      </div>
    </div>

    <div class="mb-6 flex items-center justify-between">
      <h2 class="text-xl font-semibold">Derniers quizzes</h2>
      <a
        href="/quizzes/create"
    ...
    <div v-if="props.recentQuizzes && props.recentQuizzes.length" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <article
      ...
      >
        <div class="flex items-start justify-between gap-4">
          <h3 class="line-clamp-2 text-base font-semibold">{{ quiz.title }}</h3>
          <span
        ...
        <div class="mt-4 space-y-3">
          <div class="flex items-center gap-2">
            <input :value="quiz.join_url" readonly class="w-full rounded bg-white/10 px-2 py-1 text-xs text-white ring-1 ring-white/15" />
            <button type="button" class="rounded bg-white/10 px-2 py-1 text-xs ring-1 ring-white/20 hover:bg-white/15" @click="navigator.clipboard.writeText(quiz.join_url)">Copier</button>
          </div>
    ...
    <div v-else class="rounded-xl border border-white/10 bg-white/5 p-8 text-center text-white/80">
      Aucun quiz récent. <a href="/quizzes" class="underline decoration-white/30 underline-offset-4 hover:text-white">Voir tous mes quizzes</a>
    </div>
  </div>
</template>
